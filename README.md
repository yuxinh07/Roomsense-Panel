...
- `汇率` 列填结算单里的「Order Rate to AUD」（AUD 行填 1，USD 行填实际汇率）
- **AUD 永远不折算**（行内填 99 也无视）；USD 没填行内汇率就用 `fx_usd_aud` 兜底
- 非 AUD / 非 USD 行**当前不支持**，营收记 0 并在看板顶部告警
- USD 缺汇率营收也记 0，**不会静默按 1 折算**（宁可让你看见数字掉了，也不给你看起来正常其实是错的营收）
